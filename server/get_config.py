...
#        self.SCRIPTDIR         = self.CONFDIR + "scripts/"
        self.PHYS_SCRIPT       = self.CONFDIR + "physical/"
        self.SERV_SCRIPT       = self.CONFDIR + "service/"
    ...
    def get_phys_on(self):
        ...
        for conf_file in self.PHYS_SCRIPTS_ON:
            script.update( self.get_config( self.PHYS_SCRIPT_ON + conf_file) )
        return script

    def get_phys_off(self):
        script = {}
        for conf_file in self.PHYS_SCRIPTS_OFF:
            script.update( self.get_config( self.PHYS_SCRIPT_OFF + conf_file) )
        return script

    def get_phys_state(self):
        script = {}
        for conf_file in self.PHYS_SCRIPTS_STATE:
            script.update( self.get_config( self.PHYS_SCRIPT_STATE + conf_file) )
        return script

    def get_serv_on(self):
        script = {}
        for conf_file in self.SERV_SCRIPTS_ON:
            script.update( self.get_config( self.SERV_SCRIPT_ON + conf_file) )
        return script

    def get_serv_off(self):
        script = {}
        for conf_file in self.SERV_SCRIPTS_OFF:
            script.update( self.get_config( self.SERV_SCRIPT_OFF + conf_file) )
        return script
