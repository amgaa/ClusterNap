...
1) **It is generic**. Other tools are mainly focused on "servers only". ClusterNap is not. It controls servers, switches, RAIDs, VMs, instances, services... pretty much whatever a user can imagine as a node.

2) **Dependency-awareness**. When there are complex dependencies among the IT resources, it is not always clear which resources/nodes we should turn-on/off in what order. ClusterNap takes care of it.  
